...
local function set_dpad_position(control, x, y)
    --- limit max movement of control knob
    control.touch_controls.dpad.y = lume.clamp(
        control.dpad_background.y + y - control.touch_controls.dpad.height / 2 + control.dpad_background.height / 2,
        control.dpad_background.y,
        control.dpad_background.bottom_border - control.touch_controls.dpad.height)

    control.touch_controls.dpad.x = lume.clamp(
        control.dpad_background.x + x - control.touch_controls.dpad.width / 2 + control.dpad_background.width / 2,
        0,
        control.dpad_background.right_border - control.touch_controls.dpad.width)
end

local function handle_dpad_touch(control, x, y)
    local half_dpad_width, half_dpad_height = control.dpad_background.width / 2, control.dpad_background.height / 2
    --- position of touch relative to dpad background, negative if the touch is left of/above center
    x = x - control.dpad_background.x - half_dpad_width
    y = y - control.dpad_background.y - half_dpad_height

    --- clamp values to have their maximum extension at half of the dimension of the dpad
    x = math.apply_epsilon(lume.clamp(x, -half_dpad_width, half_dpad_width), 0.0005)
    y = math.apply_epsilon(lume.clamp(y, -half_dpad_height, half_dpad_height), 0.0005)

    --- scale the x value to be at maximum the last dt
    control.state.x = (x / half_dpad_width) * control.last_dt
    control.state.y = (y / half_dpad_height) * control.last_dt
...
touch_control.new = function(enable_move_pad)
    ...
    if enable_move_pad then
        ...
        ctrl.dpad_background.shape = ctrl.touch_collider:rectangle(ctrl.dpad_background.x,
    ...
    for control_type, texture in pairs(control_textures) do
        ...
        new_control.width, new_control.height = new_control.texture:getDimensions()
